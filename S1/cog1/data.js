...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		// Initialize normal array.
		// Loop over polygons.
		for(var poly = 0; poly < this.polygonVertices.length; poly++){
			var polygon = this.polygonVertices[poly];
			...
			for(var v = 0; v < polygon.length; v++){
				...
				if(polygon[v] < this.vertexNormals.length){
					vec3.add(this.vertexNormals[polygon[v]], this.polygonNormals[poly]);
				}
			}
			// Normalize normals.
